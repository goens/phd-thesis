...
Consider the example in Listing~\ref{listing:yauhau_friends_of}. It lists the same request as Listing~\ref{listing:haxl_friends_of} in \"{Y}auhau.
The \texttt{ff} operator refers to a \emph{foreign function}\index{\"{Y}auhau ! foreign function}, which is any (possibly stateful) function.
The \texttt{io} operator, on the other hand, is an explicit annotation that a function does \ac{I/O}\index{\"{Y}auhau ! io}.

\begin{listing}
\begin{flalign*}
& \lit*{let}~\lit*{numCommonFriends} = & \\ 
& \quad \lit*{ff}_\text{length}(\lit*{ff}_\text{intersect}( & \\ 
& \quad \quad \lit*{io}(\lit*{ff}_\text{friendsOf}(x)),~\lit*{io}(\lit*{ff}_\text{friendsOf}(y)) & \\
& \quad ))~\lit*{in}~\ldots & 
\end{flalign*}
