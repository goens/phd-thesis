...
In Line 5, the \texttt{param} channel is read and the \texttt{loop\_count} parameter extracted.
Inside the scope from Lines 5-7, the \texttt{param} variable has the value of the token read.
Then in Lines 9-11 the process applies an \acs{FFT} to the data in its incoming channel \texttt{time\_coeff} and outputs it to an outgoing channel, \texttt{freq\_coef}. 
